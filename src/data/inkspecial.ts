import { Article } from '@/types';

export const InkSpecialArticles: Article[] = [
    {
        id: 10000301,
        slug: "sir-2026-voter-list-revision-telangana-guide",
        title: "స్పెషల్ ఇంటెన్సివ్ రివిజన్ (S.I.R - 2026): ఓటరు జాబితా ప్రక్షాళన - సమగ్ర మార్గదర్శకాలు",
        date: "June 30, 2026",
        category: "InkSpecial",
        categoryName: "Ink Special",
        imageUrl: "https://blogger.googleusercontent.com/img/a/AVvXsEiE-gEobpP9eWZSj0g8FzKCIJmPQECrRjhQXVR1x-5lOi6nnlw3yO3uJYna1yzErbFAU6NKNwPzfwvoxoFJnGDJSQ3r7xozWcagNzvaP86MRR3nD1d3ELoLA95045CyQgJ7B8Ap96c0L0QAJ4MFBfb57U-MnNjVfHkkOaToxYn_AM5ARa579UZcH9smlR3_",
        excerpt: "తెలంగాణలో 22 ఏళ్ల తర్వాత జరుగుతున్న ఇంటింటి సర్వే. మీ ఓటును కాపాడుకునేందుకు ఫారాలు, యాప్స్, ఆన్‌లైన్ విధానం గురించి పూర్తి వివరాలు.",
        content: ``,
        isTopStory: false,
        isLatestStory: true
    }
    ,
    {
        id: 10000302,
        slug: "telangana-gurukul-schools-education-revolution",
        title: "వర్గాల వారిగా స్కూళ్లు, కాలేజీలకు.. ఆనాటి వివక్షే కారణం.",
        date: "July 3, 2026",
        category: "InkSpecial",
        categoryName: "Ink Special",
        imageUrl: "https://blogger.googleusercontent.com/img/a/AVvXsEgVU18iIVzPoCKT69tyg0RJJ4Q0pooeHahvUmCkl7B2If6_fPUSu_3Y7NiU5JYdOYkDdVs8BXwjw52E1ghyFWW0nUbU-rUNPklBY3a5gOfzPihqykqIkzHcDlRd4XhjO3R4Vjnv26E-DPAcc1FO6gqZqC3sITNSoR7FHBbSGnHwm5B-OzUQMzbph8czfr_u",
        excerpt: "బలహీన వర్గాల బిడ్డల భవిష్యత్ కు బంగారు బాట వేసింది.. ఆనాటి వర్గాల గురుకులాలే.",
        content: ``,
        isTopStory: true,
        isLatestStory: false
    },
    {
        id: 10000303,
        slug: "revanth-reddy-20-years-political-journey",
        title: "ZPTC to CM: తెలంగాణ ఫైర్ బ్రాండ్ సీఎం రేవంత్ రెడ్డి 20 ఏళ్ల రాజకీయ ప్రస్థానం",
        date: "July 4, 2026",
        category: "InkSpecial",
        categoryName: "Ink Special",
        imageUrl: "https://blogger.googleusercontent.com/img/a/AVvXsEjKEt7shIspo2XCuwK5hOZ381yICcVO-CFO00zTVTXrXdJT-pmD7FgAYkEWMBB7DdxyZeiBLGmw0oFPG5IC_xLdLj2hldvC124dVRv9CZkbOqReJZpsN39eNsCFXow7WjeRqXeIb5XpAvC4qpnVhficdLuouQvyBW-8ELjtvDzaRA3yLptPKsnzU-UdHWmh",
        excerpt: "ఒక సాధారణ జడ్పీటీసీగా మొదలై.. దొరల గడీలను బద్దలు కొట్టి తెలంగాణ ముఖ్యమంత్రి స్థాయికి ఎదిగిన శ్రీ అనుముల రేవంత్ రెడ్డి గారి 20 ఏళ్ల స్ఫూర్తిదాయక రాజకీయ ప్రస్థానం.",
        content: ``,
        isTopStory: true,
        isLatestStory: false
    },
    {
        id: 10000304,
        slug: "ink-special-series-the-quota-chronicles-reservations",
        title: "Chapter 1: 'భారతీయ రిజర్వేషన్ల పై సమగ్ర ప్రశ్నావళి!",
        date: "July 15, 2026",
        category: "InkSpecial",
        categoryName: "Ink Special",
        imageUrl: "https://blogger.googleusercontent.com/img/a/AVvXsEgFQRHr1EiR63gPpFBY8QZpKm8yrOD4Zm1eoK4Ix4-u4gf6m6IM0ta6eORdOSWFrDcJNDZycdfKqe8EIX5PQNkjB8tt1RELsQvwu1YZ4Csyrov6YH8VTsDwhHDbZ_lK5tmoEhwqrJC45IlWRvnWNZmCO5kMci1xhtLQyu1A-kmNTHVHs9OQD4fEvBHVaD2j", 
        excerpt: "రిజర్వేషన్ల సాంకేతిక నిబంధనలు, తెలంగాణ జోనల్ వ్యవస్థ మరియు నేటి సామాజిక వాస్తవాలపై ఒక పరిశోధనాత్మక వెబ్ స్టోరీ.",
        content: ``, 
        isTopStory: true,
        isLatestStory: true
    }
];
