...
import { MessageCircle, PenTool, Clock } from 'lucide-react';
import { brandClasses } from '@/utils/theme';
import { getAllNews } from '@/data';

// Temporary mock data. You can move this to your @/data file later.

export default function MyCommentsPage() {

  const comments = getAllNews();
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 animate-in fade-in duration-500">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-10 border-b border-gray-800 pb-4">
          <MessageCircle className={`w-8 h-8 ${brandClasses.textGold}`} />
          <div>
            <h2 className="text-3xl font-serif font-bold text-white">My Comments</h2>
            <p className="text-gray-400 text-sm mt-1">Personal insights, observations, and casual thoughts.</p>
          </div>
        </div>
        ...
        <div className="space-y-8">
          {comments.map((comment) => (
            <div
              key={comment.id}
              className="bg-[#111111] border border-gray-800 rounded-xl p-8 hover:border-[#D4AF37]/50 transition-colors shadow-md relative"
            >
              {/* Decorative Gold Bar */}
